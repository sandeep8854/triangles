...
function clickHandlerFunc() {
  ...
  if (base_.value != "" && height_.value != "") {
    if (Number(base_.value) > 0 && Number(height_.value) > 0) {
      ...
      answer.innerText = `The hypotenuse of the given base: ${
        base_.value
      } and height: ${height_.value} is ${find_hypotenuse.toFixed(2)}`;
    } else {
    ...
    answer.style.backgroundColor = "#F32424";
    answer.innerText = `Please enter both base and height to get the Hypotenuse.`;
  }
}
